...
if (!isset($_GET['id_inicio'])) {
    $sql = "SELECT id, factorV, factorE, factorR, factorN, factorF, puntajeTotal, fecha, tipo_usuario 
            FROM test_pma 
...
// Modo 2: obtener resultados por ID
if (!isset($jerarquia)) {
...
$stmt->bind_param("iss", $idInicio, $jerarquia, $jerarquia);
$stmt->execute();
$resultado = $stmt->get_result();
$resultados = $resultado->fetch_assoc();

if (!$resultados) {
    error_log("➡️ PMA ID: $idInicio - No se encontraron resultados");
